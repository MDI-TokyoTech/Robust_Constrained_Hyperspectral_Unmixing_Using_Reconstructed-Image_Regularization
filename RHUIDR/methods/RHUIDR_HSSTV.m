...
function results = RHUIDR_HSSTV(A, Y, params, sizes)
...
Dhsi = @(z) cat(4, Dv(Db(z)), Dh(Db(z)), omega * Dv(z), omega * Dh(z));
Dhsit = @(z) Dbt(Dvt(z(:,:,:,1))) + Dbt(Dht(z(:,:,:,2))) + omega * Dvt(z(:,:,:,3)) + omega * Dht(z(:,:,:,4));
...
gamma_1 = 1/(9 + (33 + 8*omega)*sig_A_max^2);
gamma_2 = 1;
gamma_3 = 1/5;
gamma_4 = 1/3;
