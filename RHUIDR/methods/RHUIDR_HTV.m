...
function results = RHUIDR_HTV(A, Y, params, sizes)
...
%% Setting functions
Dhsi = @(z) cat(4, z([2:end, end],:,:) - z, z(:,[2:end, end],:)-z);
Dhsit = @(z) [-z(1,:,:,1); - z(2:end-1,:,:,1) + z(1:end-2,:,:,1); z(end-1,:,:,1)] ...
            +[-z(:,1,:,2), - z(:,2:end-1,:,2) + z(:,1:end-2,:,2), z(:,end-1,:,2)];
Mat2HSI = @(z) reshape(z',n1,n2,l);
HSI2Mat = @(z) reshape(z,n1*n2,l)';
K = @(z) Dhsi(Mat2HSI(z));
Kt = @(z) HSI2Mat(Dhsit(z));
...
for i = 1:num_iter_max
    ...
    Z3temp = ProxTVL12norm(Z3/gamma_4,lambda3/gamma_4); 
